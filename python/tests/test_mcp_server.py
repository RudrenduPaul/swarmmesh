...
async def test_register_agent_tool_calls_through_to_live_server(live_server):
    ...
    result = await server.call_tool("register_agent", {"agent_id": "mcp-agent", "role": "worker"})
    assert result.is_error is False
    assert result.structured_content["agent_id"] == "mcp-agent"
    assert result.structured_content["role"] == "worker"
...
async def test_publish_and_get_context_tools_round_trip(live_server):
    ...
    publish_result = await server.call_tool(
        "publish_context",
        {"namespace": "demo", "key": "phase", "value": "planning", "agent_id": "agent-1"},
    )
    assert publish_result.is_error is False
    assert publish_result.structured_content["value"] == "planning"

    get_result = await server.call_tool("get_context", {"namespace": "demo", "key": "phase"})
    assert get_result.is_error is False
    assert get_result.structured_content["value"] == "planning"
...
async def test_write_and_query_memory_tools_round_trip(live_server):
    ...
    assert len(query_result.structured_content["results"]) == 1
...
async def test_get_status_tool_reflects_live_server_state(live_server):
    ...
    await server.call_tool("register_agent", {"agent_id": "status-agent", "role": "worker"})
    status_result = await server.call_tool("get_status", {})
    assert status_result.is_error is False
    assert status_result.structured_content["agent_count"] >= 1
